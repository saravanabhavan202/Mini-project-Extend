...
const char *password = "SARO2002";
const char* device_token  = "2e314fdb";
//************************************************************************
String getData, Link;
String URL = "http://192.168.127.57/biometricattendancev2/getdata.php"; //computer IP or the server domain
//************************************************************************
...
// Time to start sending SMS (9:30 AM)
int targetHour = 9;
int targetMinute = 30;
bool smsSentToday = false;  // Prevents duplicate SMS sending


/////////////////////////////////////////////////////////////////////////
#include <SoftwareSerial.h>
SoftwareSerial sim800l(10, 11); // RX, TX pins (adjust as needed)
//**********************************************************************
...
void setup() {
  ...
  //Timers---------------------------------------
    ...
    if (!smsSentToday && (currentHour > targetHour || (currentHour == targetHour && currentMinute >= targetMinute))) {
        Serial.println("Time reached! Checking absentees...");
        checkForAbsentees();
        smsSentToday = true;  // Prevent duplicate SMS sending
    }
...
//*********************************************************************************
  ...
  t3 = timer.setInterval(32400000L, checkForAbsentees);  // 9:00 AM (9 hrs = 32400000 ms)
  //---------------------------------------------
  CheckMode();
}
//************************************************************************
...
if (FingerID > 0) { // Valid fingerprint detected
        markAttendance(FingerID); // Mark as present
    }
}

// Function to mark student present in the database
void markAttendance(int id) {
    if(WiFi.isConnected()) {
        HTTPClient http;
        String getData = "?FingerID=" + String(id) + "&device_token=" + String(device_token);
        ...
        Serial.println(httpCode);  // Log response
        Serial.println(payload);   // Log server response
        
        http.end();
    }
}
//check for absentees*******************************
void checkForAbsentees() {
    Serial.println("Checking for absent students...");

    if(WiFi.isConnected()) {
        HTTPClient http;
        String getData = "?FingerID=true&device_token=" + String(device_token);
        String Link = URL + getData;
        ...
        if (httpCode == 200 && payload.length() > 0) {
            sendAbsentNotification(payload);  // Send SMS if there are absentees
        }
         if (payload == "Absent users notified!") {
      Serial.println("Parents have been notified.");
    } else if (payload == "No absentees today.") {
      Serial.println("No students absent.");
    }
...
void sendAbsentNotification(String absentees) {
    Serial.println("Sending SMS to parents...");

    // Parse absentees list (format: "ID1,Name1,Phone1;ID2,Name2,Phone2;...")
    int index = 0;
    while ((index = absentees.indexOf(";")) != -1) {
        String studentData = absentees.substring(0, index);
        absentees = absentees.substring(index + 1);

        int comma1 = studentData.indexOf(",");
        int comma2 = studentData.indexOf(",", comma1 + 1);

        String FingerID = studentData.substring(0, comma1);
        String studentName = studentData.substring(comma1 + 1, comma2);
        String parentPhone = studentData.substring(comma2 + 1);

        sim800l.println("AT+CMGF=1"); // Text mode
        delay(1000);
        
        sim800l.println("AT+CMGS=\"" + parentPhone + "\"");
        delay(1000);
        
        sim800l.print("Dear Parent, your child " + studentName + " is absent today.");
        sim800l.write(26); // End SMS
        delay(2000);
    }
}
//*Send SMS with Retry Mechanism***************
void sendSMS(String phoneNumber, String studentName) {
    String message = "Dear Parent, your child " + studentName + " is absent today.";
    ...
    for (int attempt = 1; attempt <= 3; attempt++) {  // Retry up to 3 times
        ...
        Serial.println("SMS Attempt " + String(attempt) + " sent.");
        ...
        if (response.indexOf("OK") != -1) {
            Serial.println("SMS successfully sent to " + phoneNumber);
            return;
        }
    ...
    Serial.println("SMS sending failed after 3 attempts.");
}
...
void SendFingerprintID( int finger ){
  ...
  //  Serial.println(user_name);
    ...
    else if (payload == "absent_notification_sent") {
      Serial.println("Absent notifications sent.");
    }
    delay(10);
